import { 
  users, inquiries, enrollments, courses, payments, settings, customStudentFees,
...
export interface IStorage {
  ...
  // Custom Student Fees
  getAllCustomStudentFees(): Promise<CustomStudentFeeWithCourse[]>;
  getCustomStudentFee(id: string): Promise<CustomStudentFeeWithCourse | undefined>;
  getCustomStudentFeeByStudent(studentName: string, contactNo: string, courseId: string): Promise<CustomStudentFee | undefined>;
  createCustomStudentFee(customFee: InsertCustomStudentFee): Promise<CustomStudentFee>;
  updateCustomStudentFee(id: string, updateData: Partial<InsertCustomStudentFee>): Promise<CustomStudentFee>;
  deleteCustomStudentFee(id: string): Promise<void>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  // Custom Student Fees operations
  async getAllCustomStudentFees(): Promise<CustomStudentFeeWithCourse[]> {
    const results = await db
      .select({
        customFee: customStudentFees,
        course: courses,
      })
      .from(customStudentFees)
      .leftJoin(courses, eq(customStudentFees.courseId, courses.id))
      .where(eq(customStudentFees.isActive, true))
      .orderBy(desc(customStudentFees.createdAt));

    return results.map(result => ({
      ...result.customFee,
      course: result.course!,
    })) as CustomStudentFeeWithCourse[];
  }

  async getCustomStudentFee(id: string): Promise<CustomStudentFeeWithCourse | undefined> {
    const [result] = await db
      .select({
        customFee: customStudentFees,
        course: courses,
      ...
      .from(customStudentFees)
      .leftJoin(courses, eq(customStudentFees.courseId, courses.id))
      .where(eq(customStudentFees.id, id));
  ...
  async getCustomStudentFeeByStudent(studentName: string, contactNo: string, courseId: string): Promise<CustomStudentFee | undefined> {
    const [customFee] = await db
      .select()
      .from(customStudentFees)
      .where(
        and(
          eq(customStudentFees.studentName, studentName),
          eq(customStudentFees.contactNo, contactNo),
          eq(customStudentFees.courseId, courseId),
          eq(customStudentFees.isActive, true)
        )
      );
    return customFee || undefined;
  }

  async createCustomStudentFee(customFee: InsertCustomStudentFee): Promise<CustomStudentFee> {
    const [createdFee] = await db
      .insert(customStudentFees)
      .values(customFee)
      .returning();
    return createdFee;
  }

  async updateCustomStudentFee(id: string, updateData: Partial<InsertCustomStudentFee>): Promise<CustomStudentFee> {
    const [updatedFee] = await db
      .update(customStudentFees)
      .set({ ...updateData, updatedAt: new Date() })
      .where(eq(customStudentFees.id, id))
      .returning();
    return updatedFee;
  ...
  async deleteCustomStudentFee(id: string): Promise<void> {
    // Get the custom fee before deleting to restore default fees
    const customFee = await this.getCustomStudentFee(id);
    
    await db.delete(customStudentFees).where(eq(customStudentFees.id, id));
    
    // Restore default fees for existing enrollments
    if (customFee) {
      await this.restoreDefaultFeesForEnrollments(customFee);
    }
  }

  // Restore default course fees when custom fees are deleted
  async restoreDefaultFeesForEnrollments(customFee: CustomStudentFeeWithCourse): Promise<void> {
    // Find existing enrollments for this student and course
    const existingEnrollments = await db
      .select()
      .from(enrollments)
      .where(
        and(
          eq(enrollments.studentName, customFee.studentName),
          eq(enrollments.contactNo, customFee.contactNo),
          eq(enrollments.courseId, customFee.courseId)
        )
      );

    // Update each enrollment's total fee back to course defaults
    for (const enrollment of existingEnrollments) {
      let defaultFee: string;
      
      if (enrollment.feePlan === "full") {
        defaultFee = customFee.course.fullFee;
      } else {
        defaultFee = customFee.course.installmentFee;
      }
  ...
  // Update existing enrollment fees when custom fees are created/updated
  async updateEnrollmentFeesForCustomFee(customFee: CustomStudentFee): Promise<void> {
    // Find existing enrollments for this student and course
    const existingEnrollments = await db
      .select()
      .from(enrollments)
    ...
    // Update each enrollment's total fee based on their fee plan
    for (const enrollment of existingEnrollments) {
      let newTotalFee: string | null = null;
      
      if (enrollment.feePlan === "full") {
        newTotalFee = customFee.customFullFee;
      } else {
        // For installment plans, try custom installment fee first, then fall back to custom full fee
        newTotalFee = customFee.customInstallmentFee || customFee.customFullFee;
      }

      // Only update if we have a custom fee and it's different from current fee
      if (newTotalFee && newTotalFee !== enrollment.totalFee) {
        await db
          .update(enrollments)
          .set({ 
