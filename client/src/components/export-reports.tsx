...
export default function ExportReports({ onClose }: ExportReportsProps) {
  ...
  const generateCSV = () => {
    ...
    if (reportType === "enrollment") {
      const headers = [
        "Student Name", "Course", "Contact", "Father Name", "Father Contact", 
        "Email", "Address", "Start Date", "End Date", "Fee Plan", 
        "Total Fee", "Paid Amount", "Balance"
      ];
      csvContent = headers.join(",") + "\n";
      
      filteredData.forEach(enrollment => {
        const paidAmount = enrollment.payments?.reduce((sum, p) => sum + parseFloat(p.amount.toString()), 0) || 0;
        const balance = parseFloat(enrollment.totalFee.toString()) - paidAmount;
        
        const row = [
          escapeCSVValue(enrollment.studentName),
          escapeCSVValue(enrollment.course?.name || 'N/A'),
          escapeCSVValue(enrollment.contactNo),
          escapeCSVValue(enrollment.fatherName),
          escapeCSVValue(enrollment.fatherContactNo),
          escapeCSVValue(enrollment.studentEmail),
          escapeCSVValue(enrollment.studentAddress),
          escapeCSVValue(enrollment.startDate),
          escapeCSVValue(enrollment.endDate),
          escapeCSVValue(enrollment.feePlan),
          escapeCSVValue(`₹${parseFloat(enrollment.totalFee.toString()).toLocaleString()}`),
          escapeCSVValue(`₹${paidAmount.toLocaleString()}`),
          escapeCSVValue(`₹${balance.toLocaleString()}`)
        ];
      ...
      const headers = [
        "Student Name", "Course", "Payment Date", "Amount", "Payment Mode", "Transaction ID"
      ];
      csvContent = headers.join(",") + "\n";
      ...
      filteredData.forEach(enrollment => {
        enrollment.payments?.forEach(payment => {
          const row = [
            escapeCSVValue(enrollment.studentName),
            escapeCSVValue(enrollment.course?.name || 'N/A'),
            escapeCSVValue(payment.paymentDate),
            escapeCSVValue(`₹${parseFloat(payment.amount.toString()).toLocaleString()}`),
            escapeCSVValue(payment.paymentMode),
            escapeCSVValue(payment.transactionId || "N/A")
          ];
