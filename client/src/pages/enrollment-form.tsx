...
export default function EnrollmentForm() {
  ...
  useEffect(() => {
    if (inquiry) {
      form.reset({
        ...
        endDate: "",
        feePlan: undefined,
        totalFee: inquiry.course.fullFee,
  ...
  const onSubmit = (data: EnrollmentFormData) => {
    // Update totalFee based on selected fee plan and ensure batchId is included
    const updatedData = {
      ...data,
  ...
  const fullFee = parseFloat(inquiry.course.fullFee);
  const installmentFee = parseFloat(inquiry.course.installmentFee);
  const feePlans = JSON.parse(inquiry.course.feePlans || '[]');
                    ...
                    <RadioGroup onValueChange={field.onChange} value={field.value} className="space-y-4">
                      ...
                      <div className="flex items-start p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-lg font-semibold text-green-600 mt-2">₹{fullFee.toLocaleString()}</div>
                        </div>
                      </div>
                      
                      {/* Installment Plans */}
                      {feePlans.map((plan: any, planIndex: number) => (
                        <div key={planIndex} className="flex items-start p-4 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50">
                          <RadioGroupItem value={`installment_${planIndex}`} id={`plan_${planIndex}`} className="mt-1 mr-3" />
                          <div className="flex-1">
                            <label htmlFor={`plan_${planIndex}`} className="font-medium text-gray-900 cursor-pointer">
                              {plan.name} - Installments
                            </label>
                            <div className="text-sm text-neutral mt-1">
                              {plan.installments && plan.installments.map((inst: any, idx: number) => (
                                <span key={idx}>
                                  {idx > 0 && " + "}₹{inst.amount.toLocaleString()}
                                </span>
                              ))}
                              {!plan.installments && <span>Single payment: ₹{plan.amount?.toLocaleString()}</span>}
                            </div>
                            <div className="text-lg font-semibold text-primary mt-2">
                              Total: ₹{installmentFee.toLocaleString()}
                            </div>
                          </div>
                        </div>
                      ))}
