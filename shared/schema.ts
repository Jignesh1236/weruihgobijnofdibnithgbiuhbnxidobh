...
// Custom student fees table for setting different fees per student
export const customStudentFees = pgTable("custom_student_fees", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  studentName: text("student_name").notNull(),
  contactNo: text("contact_no").notNull(),
  courseId: varchar("course_id").references(() => courses.id).notNull(),
  customFullFee: decimal("custom_full_fee", { precision: 10, scale: 2 }),
  customInstallmentFee: decimal("custom_installment_fee", { precision: 10, scale: 2 }),
  customInstallment1: decimal("custom_installment1", { precision: 10, scale: 2 }),
  customInstallment2: decimal("custom_installment2", { precision: 10, scale: 2 }),
  customFeePlans: text("custom_fee_plans"), // JSON string containing custom installment plans
  reason: text("reason"), // Reason for custom fee (scholarship, discount, etc.)
  isActive: boolean("is_active").default(true),
  createdBy: text("created_by").notNull(), // Admin who created this custom fee
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
export const coursesRelations = relations(courses, ({ many }) => ({
  ...
  customStudentFees: many(customStudentFees),
}));

export const customStudentFeesRelations = relations(customStudentFees, ({ one }) => ({
  course: one(courses, {
    fields: [customStudentFees.courseId],
    references: [courses.id],
  }),
}));
...
export const insertCustomStudentFeeSchema = createInsertSchema(customStudentFees).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});
...
export type InsertCustomStudentFee = z.infer<typeof insertCustomStudentFeeSchema>;
export type CustomStudentFee = typeof customStudentFees.$inferSelect;
...
export type CustomStudentFeeWithCourse = CustomStudentFee & {
  course: Course;
};
